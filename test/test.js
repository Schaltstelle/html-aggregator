...
describe('template', () => {
    ...
    it('should load yaml', () => {
        return template.run('{{#each (globYaml "test/data.md test/*.yaml")}}({{text}}-{{{content}}}){{/each}}').then(res => {
            assert.equal(res.data, fs.readFileSync('test/expected-yaml-data.html', 'utf8'))
        })
    })
})
